...
def merge_image_by_resizing(frames: Tuple[np.ndarray, ...]) -> np.ndarray:
    ...
    resized_frames = [cv2.resize(
        frame, (int(target_h * frame.shape[1]/frame.shape[0]), target_h)) for frame in frames]
    # print(f"resized_images: {int(target_h * frames[1].shape[1]/frames[1].shape[0])}, {target_h}")
